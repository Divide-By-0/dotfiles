...
- Empty pane titles are saved as `(untitled)`. Otherwise Bash's tab-delimited `read` shifts the cwd, active flag, command and PID into the wrong columns and loses the relaunch command. The installer applies this small tmux-resurrect patch; it does not replay arbitrary shell commands or recover already-lost checkpoints.
